...
from app.mongodb.service import MongoDBService, get_mongo_service
...
async def upload_image(
        ...
        current_user: dict = Depends(get_current_user), service: MongoDBService = Depends(get_mongo_service)
        ):
    ...
    image_data = ImageCreate(
            filename = file.filename, description = description, content = content
            )
    
    file_id = await service.create_image(image_data, current_user["id"])
...
async def download_image(
    ...
    return StreamingResponse(
            io.BytesIO(image_data["content"]), media_type = "image/jpeg",
...
async def download_document(
    ...
    return StreamingResponse(
            io.BytesIO(doc_data["content"]), media_type = "application/octet-stream",
