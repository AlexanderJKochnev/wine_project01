...
from pydantic import BaseModel as BaseOrigin, ConfigDict, Field, model_serializer
...
class TextArea(BaseOrigin):
    ...
    model_config = ConfigDict(json_schema_extra={"example": "Определи язык оригинала и переведи "
                                                            "текст \"{phrase}\" на {lang} "
                                                 "язык. Данный текст относится к области \"{drink}\" "
                                                            "- обязательно подбирай слова из соответствующего "
                                                            "словаря, используй устоявшийся эквивалент на {lang} языке. Только при отсутствии эквивалента или подходящего словарного слова - транслитерируй. Переводи строго, без пояснений. Обращай внимание на согласование родов. Жесткое условие для фактов: {translation_hints}. Категорически запрещено писать вводные слова, вступление, здороваться, комментировать или объяснять свое решение, выдумывать несуществующие сущности. Твой ответ должен начинаться сразу с перевода. Перевод «"})
