...
def init_postgres_connection():
    try:
        conn = psycopg2.connect(
                host = st.secrets["POSTGRES_HOST"], port = st.secrets["POSTGRES_PORT"],
                database = st.secrets["POSTGRES_DB"], user = st.secrets["POSTGRES_USER"],
...
def init_mongo_connection():
    try:
        client = pymongo.MongoClient(
                host = st.secrets["MONGO_HOST"], port = int(st.secrets["MONGO_PORT"]),
                username = st.secrets["MONGO_USER"], password = st.secrets["MONGO_PASSWORD"], authSource = 'admin'
                )
        return client
    except Exception as e:
...
def show_api_integration():
    ...
    fastapi_url = st.secrets.get("FASTAPI_URL", "http://app:8000")
